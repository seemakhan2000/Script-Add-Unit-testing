...
// Make sure User is correctly imported and mocked
jest.mock('./script', () => {
  const actualScript = jest.requireActual('./script');
  return {
    ...actualScript,
    User: {
      ...
      deleteMany: jest.fn().mockResolvedValue({ deletedCount: 100 }),
      collection: {
        createIndex: jest.fn(), // Ensure createIndex is mocked
        indexes: jest.fn().mockResolvedValue([]), // Mock indexes if needed
      },
    },
    deleteUsers: jest.fn(),
...
describe('Mock MongoDB Connection', () => {
  it('should mock MongoDB connection successfully', async () => {
    await connectDB();
   
  });
});

describe("Unit Test for createIndexes Function", () => {
  it("should handle errors when creating indexes", async () => {
    // Ensure createIndex is mocked
    jest.spyOn(script.User.collection, 'createIndex').mockRejectedValue(new Error('Index creation error'));

    try {
      await script.createIndexes();
    } catch (error) {
      // Ensure the error is handled correctly
      expect(error.message).toBe('Index creation error');
    }
  });
});
...
describe("Unit Tests for Helper Functions", () => {
  ...
  it("should populate the database with random users", async () => {
    await script.populateDatabase({}, { status: jest.fn().mockReturnThis(), json: jest.fn() });

    const mockInsertMany = script.User.insertMany;
    const insertedUsers = mockInsertMany.mock.calls.flat();
...
describe("Unit Test for getUsers Function", () => {
  it("should get users with pagination", async () => {
    ...
    const mockUsers = [
      { _id: "1", username: "User1", email: "user1@example.com", phone: "1234567890" },
      { _id: "2", username: "User2", email: "user2@example.com", phone: "0987654321" },
    ];
    ...
    script.User.find.mockReturnValue(mockQuery);
    script.User.countDocuments.mockResolvedValue(40);
...
describe('Unit Test for deleteUser Function', () => {
  ...
  it("should delete a user successfully", async () => {
    ...
    const mockUser = { _id: "12345", username: "User1", email: "user1@example.com", phone: "1234567890" };
    script.User.findByIdAndDelete.mockResolvedValue(mockUser);
    await script.deleteUsers(mockRequest, mockResponse);
  ...
  it("should return a 404 if the user is not found", async () => {
    const mockRequest = { params: { id: "12345" } };
    const mockResponse = { status: jest.fn().mockReturnThis(), json: jest.fn() };
    script.User.findByIdAndDelete.mockResolvedValue(null);
    await script.deleteUsers(mockRequest, mockResponse);
  ...
  it("should handle errors gracefully", async () => {
    const mockRequest = { params: { id: "12345" } };
    const mockResponse = { status: jest.fn().mockReturnThis(), json: jest.fn() };
    script.User.findByIdAndDelete.mockRejectedValue(new Error("Database error"));
    await script.deleteUsers(mockRequest, mockResponse);
